...
3. Skip a commit if the effect is not visible to the user. Do not add an entry for that commit. If the commit sounds technical and internal but it changes something for the user, include it. Example: A commit adding "a pre-run check against the already-read-books file" sounds like internal implementation detail, but it changes which books the recommender shows; include it, don't skip it.
4. When a skill silently defaults to something, say that a default was applied.
5. Format release notes answers like this: "Commit 20 (30aace2): Sentence here."
6. Do not check any repo for commits before generating release notes. Use only the input the user gives you to generate release notes.
